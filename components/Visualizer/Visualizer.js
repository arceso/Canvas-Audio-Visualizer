...
export default class Visualizer {
  constructor(media, canvas, cover, colors, circleSize = 100, vibration, normalize, coverVibrates) {
    this.media = media;
    this.canvas = canvas;
    this.cover = cover;
    this.colors = colors;
    this.circleSize = circleSize;
    this.normalize = normalize ? normalize : this.normalize;
  ...
  renderFrame = () => {
    ...
    let circleShadow = new paper.Shape.Circle(view.center, 25 * Math.log(2 * shadowWidth));
    ...
    if (this.vibration && this.DATA_ARRAY.every(level => level > 50)) {
      const randomX = this.vibration[0] - this.getRandomInt(0, this.vibration[0] * 2),
        randomY = this.vibration[1] - this.getRandomInt(0, this.vibration[1] * 2);
  ...
  normalize = (current, inner) => Math.pow(current, 1 / (inner ? 2.25 : 2)) * 8;

  getEndOfLine = (size, degrees) => view.center.add([(size + this.circleSize) * this.cos(degrees), (size + this.circleSize) * this.sin(degrees)])

  getEndOfCurve = (currentAngle) => view.center.add([
    this.cos(this.bisection(currentAngle)) * this.circleSize,
    this.sin(this.bisection(currentAngle)) * this.circleSize
  ])

  bisection = (currentAngle) => currentAngle + this.DEGS_PER_LINE / 2

  onResize = () => view.viewSize = new Size(window.innerWidth, window.innerHeight);

  sin = number => this.roundNumber(Math.sin(this.toRadians(number)))

  cos = number => this.roundNumber(Math.cos(this.toRadians(number)))

  toRadians = degree => degree * (Math.PI / 180)

  roundNumber = (number, decimals = 5) => Math.round(number * Math.pow(10, decimals)) / Math.pow(10, decimals)
